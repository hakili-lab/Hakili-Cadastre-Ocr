...
- Frontend: `useTranscribe.ts`'s `start(file)` calls `getPdfPageCount` (cheap, `pdf-lib`, no rasterization) to decide `/pdf/start` vs. the chunked path; the chunked path (`startPdfChunkedFlow`) opens the job, then `for`-loops `await uploadPdfChunk(...)` per chunk. Once `pdfJobId` is set (right after the job opens, before any chunk is sent), the **existing** `statusQuery` polling takes over unchanged — `progress`/`data` already reflect real accumulating progress while the chunk-sending loop is still running, no new UI plumbing needed. Two additive-only pieces of state (`isChunkedStarting`, `chunkUploadError`) fold into the *existing* `isPending`/`isError`/`error` fields of `UseTranscriptionResult` — the hook's returned shape is unchanged, so `LoadingScreen`/`ResultScreen` needed zero modifications.
