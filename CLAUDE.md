...
**Known limitation**: `job_store.py`'s job dict is process-local memory — fine for a single dev/personal-use `uvicorn` process, but would lose jobs on restart and wouldn't work across multiple workers/processes (would need Redis or similar for that).

Config (`app/config.py`) is loaded once via `@lru_cache` from environment variables (`.env` via `python-dotenv`): `ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL` (default `claude-sonnet-5`), `MAX_IMAGE_SIZE_MB` (default `5`), `MAX_TOKENS` (default `4096` in code — but `.env.example` sets it to `8192`, which is what a fresh `.env` copy will actually use).

Error handling convention in the router: `ValueError` → 400/422 depending on stage, `RuntimeError` (missing API key) → 500, `anthropic.APIError` → 502, anything else → 500. PDF size limit is 4x the image limit (matches the frontend's client-side check: 5 MB for images, 20 MB for PDFs in `UploadScreen.tsx`).
...
- Storage is SQLite (stdlib `sqlite3`, no new dependency) plus PNG files on disk, under `ocr-math-api/data/` (gitignored, path anchored on `__file__` so it doesn't depend on the cwd `uvicorn` is launched from) — consistent with the same "single-process, personal use" assumption as `job_store.py`.
...
- `app/routers/corrections.py` exposes `POST /corrections` as multipart (`image` file + form fields), reusing `validate_content_type`/`validate_size` from `image_utils.py` and rejecting an empty error description.
...
1. **Blocking event-loop calls (highest priority)** — `normalize_orientation`, `resize_for_vision` (PIL) and `convert_pdf_to_images` (PyMuPDF) are called synchronously inside `async def` route handlers in `transcription.py`, unlike `correction_store.save_correction` which already goes through `asyncio.to_thread` in `corrections.py`. Under concurrent load, each resize/rasterization call stalls the whole event loop — including PDF status polling for other users. Fix: wrap these three calls in `await asyncio.to_thread(...)`, same pattern as `corrections.py`.
...
3. **`job_store.py` unbounded growth** — the in-memory job dict is never purged, so it grows without bound as long as the process runs. Fix: periodic or on-write eviction of jobs in `"done"`/`"error"` state older than some TTL (a few hours). Separately, the store is process-local by design — at this scale that's fine as long as the deployment is pinned to a **single instance, vertically scaled** (no autoscaling/multiple replicas), not something to fix by adding Redis prematurely.
...
**Critical:**
1. **Upload fully buffered into memory before size validation** — in all three upload endpoints (`routers/transcription.py`: `transcribe_image` and `start_pdf_transcription`; `routers/corrections.py`: `create_correction`), `await file.read()` (or `image.read()`) loads the *entire* request body into memory before `validate_size()` ever runs. A client can send an arbitrarily large body and exhaust server memory before the declared 5 MB/20 MB limit is checked — nothing currently caps this at the network/proxy level either. Fix: a streaming size check (abort once the byte count exceeds the limit, before buffering the rest) and/or a reverse-proxy body-size cap (`client_max_body_size` in nginx or equivalent) once deployed behind one.
2. **No cap on PDF page count** — `convert_pdf_to_images` (`utils/image_utils.py`) loops over every page of the uploaded PDF with no upper bound. A PDF well under the 20 MB limit can still contain thousands of near-empty pages, each triggering a PyMuPDF rasterization *and* a paid Anthropic call — a cost-bomb/DoS vector. Fix: a `MAX_PDF_PAGES` setting, rejected with 400 before processing starts.
3. **No `.dockerignore`** — must be created *before* any Dockerfile, otherwise a naive `COPY . .` would bake the real `.env` (live `ANTHROPIC_API_KEY` and `APP_API_KEY`) and `data/` (SQLite + user-submitted image crops) directly into image layers — a real secret/data leak if that image is ever pushed anywhere, even a "private" registry.

**Important:**
4. **No rate limiting on any endpoint** — combined with the `VITE_APP_API_KEY` limitation above, nothing stops a client who has extracted the key from the shipped frontend bundle from hammering `/transcribe` — direct, uncapped cost exposure via the Anthropic API. Needs a per-key/per-IP rate limit (e.g. `slowapi`, or handled at the reverse-proxy/CDN level).
5. **Generic exception handlers leak internal error text to the client** — e.g. `detail=f"Erreur inattendue : {exc}"` in `transcription.py`/`corrections.py` returns the raw Python exception message in the HTTP response instead of logging it server-side only and returning a generic message.
6. **Non-constant-time API key comparison** — `provided_key != settings.APP_API_KEY` in `app/security.py` is a theoretical timing-attack surface. Fix: `secrets.compare_digest(...)` instead of `!=`, cheap to apply.
7. **Dependencies unpinned, never audited** (see also the earlier backend audit above) — worth running `pip-audit` once before freezing a Docker image; a vulnerable transitive dependency is harder to patch once baked into a shipped image than in a dev environment.
...
9. Multi-stage build — don't ship build tools/pip cache/dev deps in the final image.
10. Pin the base image tag (and ideally digest), not a floating `python:3.x`.
11. `data/` must be a mounted volume, never copied into the image (same persistent-disk requirement already noted in the deployment discussion above).
12. Vite bakes `VITE_*` vars in at *build* time — a single built image can't be reused across environments (dev/staging/prod) with different backend URLs unless values are injected at container *startup* (e.g. an entrypoint script substituting a placeholder in the built JS/HTML) rather than at `docker build` time. Decide up front whether that flexibility is needed.

**Minor, not urgent:**
13. `allow_credentials=True` in the CORS middleware isn't needed (auth is header-based, not cookie-based) — harmless as-is but superfluous.
14. No security headers (CSP, etc.) — normally the responsibility of whatever sits in front of the app (nginx/Vercel/Railway), not the FastAPI app itself.
